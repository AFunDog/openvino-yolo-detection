...
if __name__ == "__main__":
    ...
    if mode == "image":
        ...
        output_path = sys.argv[3] if len(sys.argv) > 3 else "output_yolov26.png"
        detect_image(image_path, output_path)

    elif mode == "camera":
        # 模式2: 检测摄像头
        camera_id = int(sys.argv[2]) if len(sys.argv) > 2 else 0
        output_path = sys.argv[3] if len(sys.argv) > 3 else "output_camera_yolov26.mp4"
        detect_video(camera_id, output_path)

    elif mode == "video":
        # 模式3: 检测视频文件 (默认)
        video_path = sys.argv[2] if len(sys.argv) > 2 else "input_video.mp4"
        output_path = sys.argv[3] if len(sys.argv) > 3 else "output_yolov26.mp4"
        detect_video(video_path, output_path)

    elif mode == "rtsp":
        # 模式4: RTSP流
        rtsp_url = sys.argv[2] if len(sys.argv) > 2 else "rtsp://your_stream_url"
        output_path = sys.argv[3] if len(sys.argv) > 3 else "output_rtsp_yolov26.mp4"
        detect_video(rtsp_url, output_path)

    else:
        print("使用方法:")
        print("  python yolov26.py image [图片路径] [输出路径]")
        print("  python yolov26.py camera [摄像头ID] [输出路径]")
        print("  python yolov26.py video [视频路径] [输出路径]")
        print("  python yolov26.py rtsp [RTSP地址] [输出路径]")
        print("\n示例:")
        print("  python yolov26.py image test.png result.png")
        print("  python yolov26.py camera 0 output.mp4")
        print("  python yolov26.py video test_video.mp4 output.mp4")
